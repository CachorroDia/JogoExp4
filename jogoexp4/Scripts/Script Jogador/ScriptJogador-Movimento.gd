...
@export var barraVida : TextureProgressBar ##Define a barra de progresso que mostra o HP do jogador
var porcentagem : int = 0;
@export var porcentagemMostrador : Label
...
@export var BulletScene : PackedScene ## Instancia a cena da bala, usada para criar com o instantiete
...
func causar_dano(dano : float ) -> void: ##Subtrai a vida do jogador e atualiza a barra de progresso
	...
	porcentagem = round( (vidaAtual / vidaMax) * 100 )
	porcentagemMostrador.text = str(porcentagem,"%")
	animador.play("Dano")
	if(vidaAtual <= 0):
		vivo = false
		visible = false
